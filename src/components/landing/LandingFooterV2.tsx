    ...
    <footer className="border-t border-border bg-slate-950 text-white">
      <section className="border-b border-white/10 bg-primary/10">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
          <div>
            <div className="text-sm font-bold uppercase tracking-[.15em] text-emerald-300">Gratis banplanerare</div>
            <h2 className="mt-2 font-display text-3xl font-semibold tracking-tight sm:text-4xl">Rita agility- och hoopersbanor utan konto.</h2>
            <p className="mt-3 max-w-2xl leading-7 text-white/65">Placera hinder i meterskala, numrera banan, se hundlinje och avstånd, kontrollera ritningsbara svenska regler och hämta inspiration från Banbanken.</p>
            <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-sm text-white/75">{["Ingen registrering för att börja","Lokal autosparning","Svensk regelcheck","Mobil, surfplatta och dator"].map(x=><span key={x} className="flex items-center gap-1.5"><Check className="h-4 w-4 text-emerald-300"/>{x}</span>)}</div>
