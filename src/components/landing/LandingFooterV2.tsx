  ...
  return (
    <footer className="border-t border-border bg-slate-950 text-white">
      <section className="border-b border-white/10 bg-primary/10">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
          <div>
            ...
            <p className="mt-3 max-w-2xl leading-7 text-white/65">Rita agility och hoopers i riktig meterskala med komplett hinderpalett, klassmallar, versionerade regelverk, banlinje, analys, PDF/export, 3D, zoom, pan och lokal autosparning. Banbanken innehåller 25 riktiga V2-kartor – inklusive 12 Nollklasskartor – som öppnas utan konvertering i editorn.</p>
            <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-sm text-white/75">{["Ingen registrering för att rita","25 färdiga V2-kartor","12 Nollklasskartor","PDF · bild · JSON · 3D"].map(x=><span key={x} className="flex items-center gap-1.5"><Check className="h-4 w-4 text-emerald-300"/>{x}</span>)}</div>
            <div className="mt-6 flex flex-wrap gap-3"><Link to="/banplanerare" className="inline-flex h-12 items-center gap-2 rounded-full bg-white px-5 font-bold text-slate-950 hover:bg-white/90"><LayoutGrid className="h-4 w-4"/>Öppna hela planaren <ArrowRight className="h-4 w-4"/></Link><Link to="/banor" className="inline-flex h-12 items-center gap-2 rounded-full border border-white/20 px-5 font-semibold hover:bg-white/5"><Library className="h-4 w-4"/>Se alla 25 banor</Link></div>
          </div>
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:grid-cols-2 sm:px-6 lg:grid-cols-4">
        <div><div className="font-display text-lg font-semibold">Agility<span className="text-emerald-300">Manager</span></div><p className="mt-3 text-sm leading-6 text-white/55">Rita först. Hitta en färdig bana. Konto när du vill synka, dela i klubb eller koppla banan till resten av din träning.</p></div>
        <div><h3 className="text-xs font-bold uppercase tracking-wider text-white/45">Banplanering</h3><div className="mt-4 flex flex-col gap-2 text-sm text-white/70"><Link to="/banplanerare">Gratis banplanerare</Link><Link to="/banor">Banbank – 25 kartor</Link><Link to="/banor/noll_2026_slalom_15x30">Nollklass Slalom 15×30</Link><Link to="/om-agility">Om agility</Link><Link to="/hoopers">Om hoopers</Link><Link to="/hoopers-regler">Hoopers-regler</Link></div></div>
