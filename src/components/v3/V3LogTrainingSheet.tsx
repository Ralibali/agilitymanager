        ...
        <div className="overflow-y-auto px-5 pb-5 space-y-5 flex-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Hund" icon={<UserIcon size={13} />} error={errors.dog_id}>
              <select
                value={form.dog_id}
                onChange={(e) => update("dog_id", e.target.value)}
          ...
          <Field label="Tränade hinder" icon={<Dumbbell size={13} />}>
            <ChipRow
              ...
              onToggle={(o) => toggleArr("obstacles", o)}
            />
          </Field>
          ...
          <Field label="Vad funkade bra?">
            ...
            />
            <div className="text-[10px] text-v3-text-tertiary text-right mt-1">
              {form.notes_good.length}/500
            </div>
          </Field>
        </div>
  ...
  return (
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map((i) => (
        ...
        >
          <Star size={compact ? 16 : 18} fill={i <= value ? "currentColor" : "none"} strokeWidth={1.6} />
        </button>
      ))}
    </div>
  );
      ...
      {options.map((o) => {
        ...
        return (
          <button
            ...
            onClick={() => onToggle(o)}
            className={cn(
              "h-8 px-3 rounded-full text-v3-xs font-medium transition-colors border",
              isOn
