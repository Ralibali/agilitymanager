...
export default function CoursePlanner3D({ obstacles, paths, widthMeters, heightMeters, initialMode = "view", courseName, onClose }: CoursePlanner3DProps) {
  ...
  const walkPose = useMemo(() => {
    ...
    return { pos: [ma.x - (dx / len) * 2, 1.65, ma.z - (dz / len) * 2] as [number, number, number], target: [mb.x, 1.65, mb.z] as [number, number, number] };
    ...
    <div className="fixed inset-0 z-[1100] bg-[#f6f2ea]" style={{ touchAction: "none" }}>
      <div className="absolute top-0 left-0 right-0 z-30 flex items-center justify-between gap-2 px-3 py-3 pt-[max(0.75rem,env(safe-area-inset-top))] bg-gradient-to-b from-black/35 to-transparent text-white">
        ...
        <div className="flex items-center gap-1 rounded-full bg-black/35 backdrop-blur p-1 shadow-lg">
          ...
          <button onClick={() => { setCurrentIdx(0); setMode("walk"); }} className={`h-9 px-3 rounded-full text-sm font-semibold inline-flex items-center gap-1.5 ${mode === "walk" ? "bg-white text-black" : "text-white/85"}`}><Footprints size={15} /> Gå banan</button>
        </div>
      </div>
      ...
      {mode === "walk" && isMobile && <MobileWalkControls joystickRef={joystickRef} lookDeltaRef={lookDeltaRef} sprintRef={sprintRef} knob={knob} setKnob={setKnob} sprinting={sprinting} setSprinting={setSprinting} showHint={showHint} setShowHint={setShowHint} />}
      {mode === "walk" && !isMobile && <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 text-white/80 text-xs bg-black/50 px-3 py-1.5 rounded-full">Klicka för att låsa muspekaren · WASD = gå · Shift = spring · Esc = lämna</div>}
    </div>
  );
}
...
function MobileWalkControls({ joystickRef, lookDeltaRef, sprintRef, knob, setKnob, sprinting, setSprinting, showHint, setShowHint }: { joystickRef: { current: { x: number; y: number } }; lookDeltaRef: { current: { x: number; y: number } }; sprintRef: { current: boolean }; knob: { x: number; y: number }; setKnob: (v: { x: number; y: number }) => void; sprinting: boolean; setSprinting: (v: boolean) => void; showHint: boolean; setShowHint: (v: boolean) => void }) {
  ...
  return <><div className="absolute inset-0 z-10" style={{ touchAction: "none" }} onTouchStart={(e) => { if (lookId.current != null) return; const t = e.changedTouches[0]; lookId.current = t.identifier; lookLast.current = { x: t.clientX, y: t.clientY }; }} onTouchMove={(e) => { for (let i = 0; i < e.changedTouches.length; i++) { const t = e.changedTouches[i]; if (t.identifier === lookId.current) { const dx = t.clientX - lookLast.current.x; const dy = t.clientY - lookLast.current.y; lookLast.current = { x: t.clientX, y: t.clientY }; lookDeltaRef.current.x += dx; lookDeltaRef.current.y += dy; } } }} onTouchEnd={(e) => { for (let i = 0; i < e.changedTouches.length; i++) if (e.changedTouches[i].identifier === lookId.current) lookId.current = null; }} onTouchCancel={() => { lookId.current = null; }} />{showHint && <button type="button" onClick={() => setShowHint(false)} className="absolute top-[170px] left-1/2 -translate-x-1/2 z-30 px-4 py-3 rounded-2xl bg-black/70 backdrop-blur-md border border-white/15 text-white text-[12px] leading-snug max-w-[88vw] shadow-2xl"><div className="font-semibold mb-1.5 text-[13px]">Så styr du</div><div className="opacity-90 text-left space-y-0.5"><div>Joystick nere till vänster styr rörelse</div><div>Dra på vyn för att titta runt</div><div>Håll Sprint för att gå snabbare</div></div><div className="mt-2 text-[10px] opacity-60">Tryck för att stänga</div></button>}<div className="absolute bottom-[calc(env(safe-area-inset-bottom)+0.25rem)] left-1/2 -translate-x-1/2 z-30 px-3 py-1 rounded-full bg-black/45 backdrop-blur text-white/75 text-[10px] pointer-events-none">Joystick · Dra för att titta · Sprint</div><div ref={padRef} className="absolute left-4 bottom-[calc(env(safe-area-inset-bottom)+2.5rem)] z-30 h-28 w-28 rounded-full bg-black/20 backdrop-blur border border-white/35 shadow-lg" style={{ touchAction: "none" }} onTouchStart={(e) => { e.stopPropagation(); const t = e.changedTouches[0]; stickTouchId.current = t.identifier; setStick(t.clientX, t.clientY); }} onTouchMove={(e) => { e.stopPropagation(); for (let i = 0; i < e.changedTouches.length; i++) { const t = e.changedTouches[i]; if (t.identifier === stickTouchId.current) setStick(t.clientX, t.clientY); } }} onTouchEnd={(e) => { e.stopPropagation(); for (let i = 0; i < e.changedTouches.length; i++) if (e.changedTouches[i].identifier === stickTouchId.current) resetStick(); }} onTouchCancel={(e) => { e.stopPropagation(); resetStick(); }}><div className="absolute h-11 w-11 rounded-full bg-white/90 shadow-lg pointer-events-none" style={{ left: "50%", top: "50%", transform: `translate(-50%, -50%) translate(${knob.x * 26}px, ${knob.y * 26}px)` }} /></div><button type="button" className={`absolute right-4 bottom-[calc(env(safe-area-inset-bottom)+2.5rem)] z-30 h-20 w-20 rounded-full backdrop-blur border shadow-lg text-white text-xs font-bold uppercase tracking-wider select-none transition-colors ${sprinting ? "bg-amber-400/85 border-amber-200 text-black" : "bg-black/25 border-white/35"}`} style={{ touchAction: "none" }} onTouchStart={(e) => { e.preventDefault(); e.stopPropagation(); sprintRef.current = true; setSprinting(true); }} onTouchEnd={(e) => { e.preventDefault(); e.stopPropagation(); sprintRef.current = false; setSprinting(false); }} onTouchCancel={() => { sprintRef.current = false; setSprinting(false); }}>Sprint</button></>;
}
