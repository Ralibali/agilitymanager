...
class Boundary extends Component<{ onClose: () => void; children: ReactNode }, { hasError: boolean }> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div role="alertdialog" aria-modal="true" aria-label="3D-vyn kunde inte laddas" className="fixed inset-0 z-[1100] bg-[#0d1410] text-white grid place-items-center p-6">
          <div className="max-w-md text-center space-y-3">
            ...
            <button autoFocus onClick={this.props.onClose} className="h-10 px-4 rounded-full bg-white text-black font-semibold">Tillbaka till 2D</button>
          </div>
        </div>
      );
    }
...
export default function LazyCoursePlanner3D(props: CoursePlanner3DProps) {
  return (
    <Boundary onClose={props.onClose}>
      <Suspense fallback={
        <div role="status" className="fixed inset-0 z-[1100] bg-[#0d1410] text-white grid place-items-center">
          <div className="text-center space-y-3">
            <div aria-hidden="true" className="h-10 w-10 mx-auto rounded-full border-2 border-white/30 border-t-white animate-spin" />
