...
import type { Obstacle3DProps } from "./Obstacle3D";
    ...
    <Billboard position={[0, height + 0.55, 0]} renderOrder={999}>
      ...
      {highlight && (
        <mesh renderOrder={998}>
          ...
          <meshBasicMaterial color="#fde68a" transparent opacity={0.62} depthTest={false} />
        </mesh>
      )}
      <mesh renderOrder={999}>
        <circleGeometry args={[radius, 32]} />
        <meshBasicMaterial color={highlight ? "#f59e0b" : "#1d6f3c"} depthTest={false} />
      </mesh>
      <mesh position={[0, 0, 0.003]} renderOrder={1000}>
...
function Shadow({ w = 1.4, d = 0.9 }: { w?: number; d?: number }) {
  return (
    <mesh position={[0, 0.006, 0]} rotation={[-Math.PI / 2, 0, 0]} renderOrder={-1} scale={[w, d, 1]}>
      <circleGeometry args={[0.5, 32]} />
      <meshBasicMaterial color="#102015" transparent opacity={0.14} depthWrite={false} />
    </mesh>
  );
}
...
function Foot({ x, z = 0, color = DARK }: { x: number; z?: number; color?: string }) {
  return (
    <mesh position={[x, 0.035 + EPS, z]} castShadow receiveShadow>
      <boxGeometry args={[0.42, 0.07, 0.44]} />
...
function StripedBar({ main = BLUE, accent = YELLOW, length = 1.24 }: { main?: string; accent?: string; length?: number }) {
  return (
    <group rotation={[0, 0, Math.PI / 2]}>
      <mesh castShadow>
        <cylinderGeometry args={[0.043, 0.043, length, 18]} />
        <meshStandardMaterial color={main} roughness={0.35} />
      </mesh>
      {[-0.42, 0, 0.42].map((y) => (
        <mesh key={y} position={[0, y, 0]}>
          <cylinderGeometry args={[0.047, 0.047, 0.18, 18]} />
...
function AdjustablePost({ x, accent = ORANGE }: { x: number; accent?: string }) {
  return (
    <group position={[x, 0, 0]}>
      <mesh position={[0, 0.52 + EPS, 0]} castShadow receiveShadow>
        <boxGeometry args={[0.075, 1.04, 0.075]} />
        <meshStandardMaterial color={WHITE} roughness={0.38} />
      </mesh>
      {[0.28, 0.45, 0.62, 0.79].map((y) => (
        <mesh key={y} position={[0.043, y, 0]} rotation={[0, Math.PI / 2, 0]}>
...
function Jump({ color }: { color?: string }) {
  return (
    <group>
      <Shadow w={1.7} d={0.9} />
      <AdjustablePost x={-0.64} />
      <AdjustablePost x={0.64} />
      <group position={[0, 0.58 + EPS, 0]}><StripedBar main={color && color !== WHITE ? color : BLUE} accent={YELLOW} /></group>
      <mesh position={[0, 0.18, -0.22]} receiveShadow>
        <boxGeometry args={[1.34, 0.04, 0.06]} />
        <meshStandardMaterial color="#d1d5db" roughness={0.58} />
      </mesh>
    </group>
  );
}

function Oxer({ color }: { color?: string }) {
  return <group><Shadow w={1.8} d={1.3} /><group position={[0, 0, -0.25]}><Jump color={color || BLUE} /></group><group position={[0, 0.08, 0.25]}><Jump color={ORANGE} /></group></group>;
}
...
function LongJump() {
  return (
    <group rotation={[0, Math.PI / 2, 0]}>
      <Shadow w={1.65} d={1.4} />
      {[-0.58, -0.18, 0.22, 0.62].map((z, i) => (
        <mesh key={i} position={[0, 0.08 + i * 0.035 + EPS, z]} castShadow receiveShadow>
          <boxGeometry args={[1.48, 0.085, 0.18]} />
          <meshStandardMaterial color={i % 2 === 0 ? WHITE : YELLOW} roughness={0.42} />
        </mesh>
      ))}
      {[[-0.84, -0.78], [0.84, -0.78], [-0.84, 0.86], [0.84, 0.86]].map(([x, z], i) => (
        <mesh key={i} position={[x, 0.16, z]} castShadow><cylinderGeometry args={[0.035, 0.035, 0.32, 10]} /><meshStandardMaterial color={ORANGE} /></mesh>
      ))}
    </group>
  );
}
...
function Wall({ color = RED }: { color?: string }) {
  return (
    <group>
      <Shadow w={1.7} d={0.75} />
      <mesh position={[0, 0.42 + EPS, 0]} castShadow receiveShadow><boxGeometry args={[1.48, 0.82, 0.32]} /><meshStandardMaterial color={color || RED} roughness={0.55} /></mesh>
      {[-0.48, 0, 0.48].map((x) => <mesh key={x} position={[x, 0.42, 0.166]}><boxGeometry args={[0.025, 0.78, 0.012]} /><meshBasicMaterial color="#ffffff" transparent opacity={0.45} /></mesh>)}
      {[0.26, 0.52, 0.78].map((y) => <mesh key={y} position={[0, y, 0.168]}><boxGeometry args={[1.42, 0.022, 0.012]} /><meshBasicMaterial color="#ffffff" transparent opacity={0.42} /></mesh>)}
    </group>
  );
}

function Tunnel({ color = BLUE, length = 3.05, radius = 0.42 }: { color?: string; length?: number; radius?: number }) {
  return (
    <group position={[0, radius + EPS, 0]} rotation={[0, Math.PI / 2, 0]}>
      <Shadow w={1.1} d={3.25} />
      <mesh rotation={[Math.PI / 2, 0, 0]} castShadow receiveShadow><cylinderGeometry args={[radius, radius, length, 36, 1, true]} /><meshStandardMaterial color={color} side={THREE.DoubleSide} roughness={0.64} metalness={0.02} /></mesh>
      {Array.from({ length: 10 }).map((_, i) => <mesh key={i} position={[0, 0, -length / 2 + (i * length) / 9]}><torusGeometry args={[radius + 0.012, 0.012, 8, 32]} /><meshStandardMaterial color={i % 2 === 0 ? LIGHT_BLUE : "#1d4ed8"} roughness={0.55} /></mesh>)}
      {[-1, 1].map((s) => <mesh key={s} position={[0, 0, (s * length) / 2 + 0.001 * s]} rotation={[0, s === 1 ? 0 : Math.PI, 0]}><ringGeometry args={[radius * 0.82, radius * 1.02, 36]} /><meshBasicMaterial color="#071426" side={THREE.DoubleSide} /></mesh>)}
    </group>
  );
}
function HoopersTunnel() { return <Tunnel color={ORANGE} length={2.4} radius={0.45} />; }

function AFrame() {
  const apexH = 1.65;
  const half = 1.42;
  const hyp = Math.sqrt(apexH * apexH + half * half);
  const angle = Math.PI / 2 - Math.atan2(apexH, half);
  const w = 1.05;
  return (
    <group rotation={[0, Math.PI / 2, 0]}>
      <Shadow w={1.35} d={3.2} />
      {[-1, 1].map((side) => <group key={side}>
        <mesh position={[0, apexH / 2, (side * half) / 2]} rotation={[side * angle, 0, 0]} castShadow receiveShadow><boxGeometry args={[w, 0.08, hyp]} /><meshStandardMaterial color={YELLOW} roughness={0.46} /></mesh>
        <mesh position={[0, 0.42, (side * half) / 2 + side * 0.78]} rotation={[side * angle, 0, 0]}><boxGeometry args={[w + 0.01, 0.065, 0.62]} /><meshStandardMaterial color={CONTACT} roughness={0.46} /></mesh>
        {[-0.5, 0, 0.5].map((offset) => <mesh key={offset} position={[0, apexH / 2 + 0.04, (side * half) / 2 + side * offset]} rotation={[side * angle, 0, 0]} castShadow><boxGeometry args={[w + 0.04, 0.035, 0.045]} /><meshStandardMaterial color="#8b5e34" roughness={0.7} /></mesh>)}
      </group>)}
      <mesh position={[0, apexH + 0.025, 0]} rotation={[0, 0, Math.PI / 2]} castShadow><cylinderGeometry args={[0.045, 0.045, w + 0.08, 14]} /><meshStandardMaterial color={DARK} /></mesh>
    </group>
  );
}

function DogWalk() {
  const top = 1.08, horiz = 1.65, ramp = 1.7, w = 0.36;
