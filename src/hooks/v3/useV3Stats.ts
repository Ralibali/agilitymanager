...
function rangeStart(range: RangeKey): string | null {
  ...
  return isoDate(new Date(Date.now() - days * MS_DAY));
}
...
function buildWeekly(sessions: SessionRow[], results: ResultRow[]): WeeklyBucket[] {
  const map = new Map<string, WeeklyBucket>();
  const touch = (d: Date) => {
    const ws = startOfWeek(d);
    const key = isoDate(ws);
    if (!map.has(key)) {
      map.set(key, { weekStart: key, label: weekLabel(ws), sessions: 0, minutes: 0, passed: 0, starts: 0 });
    }
    return map.get(key)!;
  };
  sessions.forEach((s) => {
    const b = touch(new Date(s.date));
    b.sessions += 1;
    b.minutes += s.duration_min ?? 0;
  });
  results.forEach((r) => {
    const b = touch(new Date(r.date));
    b.starts += 1;
...
function buildScoreTrend(sessions: SessionRow[]): ScoreTrend[] {
  // Gruppera per vecka, snitta scores
...
function buildInsights(
  ...
  typeDist: TypeDistribution[],
  scoreTrend: ScoreTrend[],
  totals: V3Stats["totals"],
): Insight[] {
  const out: Insight[] = [];

  // 1. Volymtrend (sista 4 vs föregående 4)
  if (weekly.length >= 6) {
    ...
    if (prev4 > 0) {
      ...
      if (delta >= 15) {
        out.push({
          id: "vol-up",
          tone: "positive",
          title: "Träningsvolymen ökar",
          body: `Senaste 4 veckor har du tränat ${last4} pass – ${Math.round(delta)}% mer än perioden innan. Bra rytm.`,
        });
      } else if (delta <= -25) {
        out.push({
          id: "vol-down",
          tone: "warning",
          title: "Volymen har sjunkit",
          body: `Du har tränat ${last4} pass senaste 4 veckorna mot ${prev4} förra. Planera in ett par korta pass nästa vecka.`,
        });
      }
    }
  }
  ...
  if (validScores.length >= 4) {
    ...
    if (lastAvg - firstAvg >= 0.5) {
      out.push({
        id: "score-up",
        tone: "positive",
        title: "Banflyt har förbättrats",
        body: `Snittet har gått från ${firstAvg.toFixed(1)} till ${lastAvg.toFixed(1)} – ditt arbete syns i siffrorna.`,
      });
    }
  }

  // 3. Träningsmix
  if (typeDist.length > 0 && typeDist[0].percent >= 0.6) {
    out.push({
      id: "mix",
      tone: "neutral",
      title: "Ensidig träningsmix",
      body: `${Math.round(typeDist[0].percent * 100)}% av passen är "${typeDist[0].type}". Variera med en annan typ för balanserad utveckling.`,
    });
  }

  // 4. Pass-rate
  if (totals.starts >= 5) {
    if (totals.passRate >= 0.7) {
      out.push({
        id: "pass-rate-good",
        tone: "positive",
        title: `${Math.round(totals.passRate * 100)}% godkända lopp`,
        body: `Av ${totals.starts} starter har ${totals.passed} gått igenom – stabilt resultat.`,
      });
    } else if (totals.passRate < 0.4) {
      out.push({
        id: "pass-rate-low",
        tone: "warning",
        title: "Låg andel godkända lopp",
        body: `${totals.passed} av ${totals.starts} starter klara. Fokusera på en specifik felkälla i nästa pass.`,
      });
    }
  }
  ...
  if (out.length === 0) {
    out.push({
      id: "default",
      tone: "neutral",
      title: "Bygger underlag",
      body: "Logga några pass och resultat till så ger vi dig skräddarsydda insikter.",
    });
  }
...
export function useV3Stats(dogId: string | null, range: RangeKey) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const sessionsQuery = supabase
        ...
        .select("id, date, duration_min, type, sport, banflyt_score, dirigering_score, overall_mood, best_time_sec, tags, dog_id")
        .eq("user_id", user.id)
        .eq("dog_id", dogId)
      ...
      const [sRes, rRes] = await Promise.all([
        since ? sessionsQuery.gte("date", since) : sessionsQuery,
        since ? resultsQuery.gte("date", since) : resultsQuery,
      ]);
  ...
  const stats = useMemo<V3Stats>(() => {
    ...
    const weekly = buildWeekly(sessions, results);
...
/**
 * Hämtar jämförelse mellan alla användarens hundar inom valt intervall.
 */
export function useV3DogCompare(range: RangeKey) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const sQ = supabase
        .from("training_sessions")
        .select("dog_id, duration_min")
        .eq("user_id", user.id);
      const rQ = supabase
        .from("competition_results")
        .select("dog_id, passed, placement")
        .eq("user_id", user.id);

      const [dogsRes, sRes, rRes] = await Promise.all([
        dogsQ,
        since ? sQ.gte("date", since) : sQ,
        since ? rQ.gte("date", since) : rQ,
      ]);
      if (cancelled) return;
      ...
      const compare: DogCompare[] = dogs.map((d) => {
        ...
        return {
          dogId: d.id,
          dogName: d.name,
          themeColor: d.theme_color ?? "hsl(var(--v3-brand-500))",
          totalSessions: ses.length,
          totalMinutes: ses.reduce((sum, s) => sum + (s.duration_min ?? 0), 0),
          passRate: res.length > 0 ? passed / res.length : 0,
          starts: res.length,
          avgPlacement: placements.length ? placements.reduce((a, b) => a + b, 0) / placements.length : null,
        };
      });
