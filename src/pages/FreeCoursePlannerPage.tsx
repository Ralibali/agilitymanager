...
const AUTH_SIGNUP = "/auth?mode=signup&redirect=/v3";
      ...
      <main>
        <section className="max-w-6xl mx-auto px-5 pt-10 pb-6 grid lg:grid-cols-[0.95fr_1.05fr] gap-8 items-center">
          <div>
            ...
            <p className="text-lg text-black/65 mt-4 max-w-xl">
              Rita en enkel träningsbana direkt i webbläsaren. När du vill spara banan, logga passet och följa utvecklingen skapar du konto och fortsätter i appen.
            </p>
          ...
          <div className="rounded-[2rem] border border-black/10 bg-white p-4 shadow-2xl shadow-black/10">
            <PlannerCanvas obstacles={obstacles} selectedId={selectedId} onSelect={setSelectedId} />
          </div>
        </section>
        ...
        <section id="verktyg" className="max-w-6xl mx-auto px-5 py-8 grid lg:grid-cols-[260px_1fr_280px] gap-4">
          ...
          <div className="rounded-[2rem] border border-black/10 bg-white p-3 shadow-xl shadow-black/5">
            <PlannerCanvas obstacles={obstacles} selectedId={selectedId} onSelect={setSelectedId} large />
          </div>
          ...
          <aside className="rounded-3xl border border-black/10 bg-white p-4 h-fit space-y-4">
            <div>
              ...
              {selected ? <p className="text-sm text-black/60 mt-1">{selected.label} · x {Math.round(selected.x)}, y {Math.round(selected.y)}</p> : <p className="text-sm text-black/60 mt-1">Klicka på ett hinder i banan.</p>}
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <button type="button" onClick={() => moveSelected(15, 0)} disabled={!selected} className="tool-btn">→</button>
              <button type="button" onClick={() => moveSelected(0, 15)} disabled={!selected} className="tool-btn">↓</button>
              <button type="button" onClick={rotateSelected} disabled={!selected} className="tool-btn col-span-2"><RotateCcw size={14} /> Rotera</button>
            </div>
            ...
            <div className="rounded-3xl border border-[#617457]/20 bg-[#f2f6ee] p-4">
              ...
              <div className="grid grid-cols-2 gap-2 mt-3">
                <button type="button" onClick={() => setLockedAction("save")} className="inline-flex h-10 items-center justify-center gap-2 rounded-2xl bg-white border border-black/10 text-sm font-medium hover:bg-black/[0.03]"><Save size={14} /> Spara</button>
                <button type="button" onClick={() => setLockedAction("export")} className="inline-flex h-10 items-center justify-center gap-2 rounded-2xl bg-white border border-black/10 text-sm font-medium hover:bg-black/[0.03]"><FileDown size={14} /> Export</button>
              </div>
              {lockedAction && (
                <div className="mt-3 rounded-2xl bg-white border border-black/10 p-3 text-sm text-black/65">
                  <strong className="text-black">{lockedAction === "save" ? "Spara bana" : "Exportera bana"}</strong> är en konto-funktion. Skapa konto så kan du fortsätta i fulla banplaneraren.
                </div>
              )}
            </div>
        ...
        <section className="max-w-6xl mx-auto px-5 py-10">
          <div className="rounded-[2rem] bg-[#172016] text-white p-6 lg:p-8 overflow-hidden relative">
            ...
            <div className="relative grid lg:grid-cols-[0.9fr_1.1fr] gap-8 items-center">
              <div>
                <div className="text-xs uppercase tracking-[0.12em] text-white/50 font-semibold">Så blir gratisverktyget till utveckling</div>
                <h2 className="font-display text-3xl lg:text-4xl mt-3">Rita banan. Träna passet. Följ utvecklingen.</h2>
                <p className="text-white/70 mt-3 max-w-lg">Banplaneraren är starten. Värdet kommer när du sparar övningen, loggar hur det gick och ser mönster i statistiken.</p>
                <Link to={AUTH_SIGNUP} className="mt-5 inline-flex h-11 items-center justify-center rounded-full bg-white px-5 text-sm font-medium text-[#172016] hover:bg-white/90">Fortsätt i appen <ArrowRight size={15} className="ml-2" /></Link>
              </div>
              <div className="grid sm:grid-cols-3 gap-3">
                <JourneyCard icon={Save} title="1. Spara bana" text="Behåll övningen och bygg ett bibliotek." />
                <JourneyCard icon={Dumbbell} title="2. Logga pass" text="Fånga känslan medan den är färsk." />
                <JourneyCard icon={BarChart3} title="3. Se statistik" text="Förstå vad som går framåt över tid." />
              </div>
            </div>
          </div>
        </section>
...
function JourneyCard({ icon: Icon, title, text }: { icon: typeof Save; title: string; text: string }) {
  return (
    <div className="rounded-3xl border border-white/10 bg-white/[0.06] p-4">
      <div className="h-10 w-10 rounded-full bg-white/10 grid place-items-center mb-3"><Icon size={17} /></div>
  ...
  return (
    <div className={cn("relative overflow-hidden rounded-[1.5rem] border border-black/10 bg-[#edf2e7]", large ? "min-h-[540px]" : "min-h-[360px]") }>
      ...
      {obstacles.map((obstacle) => {
        ...
        return (
          <button key={obstacle.id} type="button" onClick={() => onSelect(obstacle.id)} className={cn("absolute grid place-items-center rounded-xl border bg-white shadow-sm transition-all", selected ? "border-[#617457] ring-4 ring-[#617457]/15 scale-105" : "border-black/15 hover:border-[#617457]/40", obstacle.type === "tunnel" ? "h-14 w-20" : obstacle.type === "weave" ? "h-10 w-24" : "h-12 w-12")} style={{ left: `${(obstacle.x / 600) * 100}%`, top: `${(obstacle.y / 460) * 100}%`, transform: `translate(-50%, -50%) rotate(${obstacle.rotation}deg)` }} title={obstacle.label}>
            <span className="text-2xl leading-none text-[#172016]">{obstacle.symbol}</span>
            {obstacle.number && <span className="absolute -right-2 -top-2 h-6 w-6 rounded-full bg-[#617457] text-white text-xs font-bold grid place-items-center">{obstacle.number}</span>}
          </button>
        );
      })}

      <div className="absolute left-4 bottom-4 rounded-full bg-white/90 border border-black/10 px-3 py-1 text-xs font-semibold text-[#617457] shadow-sm inline-flex items-center gap-1.5">
        <CheckCircle2 size={12} /> Gratis demo
      </div>
