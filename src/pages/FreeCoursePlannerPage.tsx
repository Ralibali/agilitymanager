    ...
    <div className="min-h-screen bg-[#f7f3eb] text-[#172016]">
      ...
      <header className="border-b border-black/10 bg-[#fffaf2]/80 backdrop-blur-xl sticky top-0 z-20">
        <div className="max-w-6xl mx-auto px-4 sm:px-5 py-3 sm:py-4 flex items-center justify-between gap-3">
          <Link to="/" className="font-display text-lg sm:text-xl font-semibold tracking-tight truncate">Agility<span className="text-[#617457]">Manager</span></Link>
          <div className="flex items-center gap-2 shrink-0"><Link to={AUTH_LOGIN} className="hidden sm:inline-flex h-10 items-center justify-center rounded-full border border-black/10 bg-white px-4 text-sm font-medium hover:bg-black/[0.03]">Logga in</Link><Link to={AUTH_SIGNUP} className="inline-flex h-10 items-center justify-center rounded-full bg-[#617457] px-4 text-sm font-medium text-white hover:bg-[#526349]">Skapa konto</Link></div>
        </div>
      </header>
      ...
      <main>
        <section className="max-w-6xl mx-auto px-4 sm:px-5 pt-8 sm:pt-10 pb-6 grid lg:grid-cols-[0.95fr_1.05fr] gap-6 lg:gap-8 items-center">
          <div className="min-w-0">
            ...
            <p className="text-base sm:text-lg text-black/65 mt-4 max-w-xl">Rita en enkel träningsbana direkt i webbläsaren. När du vill spara banan, logga passet och följa utvecklingen skapar du konto och fortsätter i appen.</p>
        ...
        <section id="verktyg" className="max-w-6xl mx-auto px-4 sm:px-5 py-6 sm:py-8 grid lg:grid-cols-[260px_1fr_280px] gap-4">
          <aside className="rounded-3xl border border-black/10 bg-white p-4 h-fit">
            <h2 className="font-display text-xl">Lägg till hinder</h2><p className="text-sm text-black/55 mt-1">Gratisversionen är begränsad till {MAX_FREE_OBSTACLES} hinder.</p>
            <div className="grid grid-cols-2 sm:grid-cols-5 lg:grid-cols-2 gap-2 mt-4">{OBSTACLES.map((item) => <button key={item.type} type="button" onClick={() => addObstacle(item.type)} className="min-h-20 rounded-2xl border border-black/10 bg-[#f7f3eb] p-3 text-left hover:border-[#617457]/40 hover:bg-[#eef2ea] transition-colors"><span className="block text-2xl leading-none">{item.symbol}</span><span className="block text-xs font-medium mt-2">{item.label}</span></button>)}</div>
