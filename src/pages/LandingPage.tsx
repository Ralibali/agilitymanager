  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <Helmet>
        ...
        <meta name="description" content="Rita agilitybanor gratis direkt i webbläsaren. Ingen inloggning krävs. Meterskala, hinder, banlinje, avstånd, svensk regelcheck, banbank och hoopers." />
        <link rel="canonical" href="https://agilitymanager.se/" />
        <meta property="og:title" content="Rita agilitybanor gratis – AgilityManager" />
        <meta property="og:description" content="Gratis banplanerare för agility och hoopers. Börja rita direkt utan konto." />
      ...
      <main>
        <section className="overflow-hidden border-b border-border bg-[radial-gradient(circle_at_80%_20%,hsl(var(--primary)/.12),transparent_38%)]">
          <div className="mx-auto grid max-w-7xl gap-10 px-4 pb-16 pt-12 sm:px-6 sm:pt-16 lg:grid-cols-[.9fr_1.1fr] lg:items-center lg:pb-24 lg:pt-20">
            <div>
              ...
              <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-sm text-muted-foreground">{["Ingen installation","Mobil & dator","Agility & hoopers","Lokal autosparning"].map(x=><span key={x} className="flex items-center gap-1.5"><Check className="h-4 w-4 text-primary" />{x}</span>)}</div>
            </div>
            ...
            <div className="relative rounded-[2rem] border border-slate-300 bg-white p-3 shadow-2xl shadow-slate-900/10">
              ...
              <div className="relative aspect-[4/3] overflow-hidden rounded-2xl border border-slate-300 bg-white" style={{backgroundImage:"linear-gradient(to right,#d9dee7 1px,transparent 1px),linear-gradient(to bottom,#d9dee7 1px,transparent 1px)",backgroundSize:"10% 12.5%"}}>
                ...
                <svg className="absolute inset-0 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none"><path d="M13 78 C20 70 20 68 26 65 S34 70 40 76 S47 65 50 58 S60 64 67 67 S78 62 80 52 S73 42 66 37 S55 31 47 33 S35 38 27 40 S22 29 18 20" fill="none" stroke="#b91c1c" strokeWidth=".7" strokeDasharray="2 1.5" opacity=".65" /><g fill="#b91c1c">{demo.slice(0,-1).map((o,i)=><polygon key={i} points={`${(o.x+demo[i+1].x)/2},${(o.y+demo[i+1].y)/2} ${(o.x+demo[i+1].x)/2-1},${(o.y+demo[i+1].y)/2-1} ${(o.x+demo[i+1].x)/2+1},${(o.y+demo[i+1].y)/2-1}`} />)}</g></svg>
        ...
        <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:py-24">
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{[
            ...
            [Copy,"Kopiera från Banbanken","Öppna en färdig bana, kopiera den och anpassa den till din egen träningsyta eller träningsgrupp."],
            [FileDown,"Ta med banan","Exportera ritningen och använd den när du bygger träningsbanan på klubben eller hemma."],
        ...
        <section className="border-y border-border bg-muted/35 py-16 sm:py-20"><div className="mx-auto max-w-7xl px-4 sm:px-6"><div className="grid gap-8 lg:grid-cols-2 lg:items-center"><div><p className="text-sm font-bold uppercase tracking-[.18em] text-primary">Banbanken</p><h2 className="mt-3 font-display text-3xl font-semibold sm:text-5xl">Slipp börja från ett tomt papper</h2><p className="mt-4 text-lg leading-8 text-muted-foreground">Banbanken ska växa till ett öppet bibliotek med agilitybanor och träningsövningar. Hitta en layout för rätt klass, kopiera den till planeringsverktyget och gör den till din egen.</p><Link to="/banplanerare?view=bank" className="mt-6 inline-flex items-center gap-2 font-bold text-primary">Utforska banorna <ArrowRight className="h-4 w-4" /></Link></div><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-3xl border border-border bg-card p-6"><Users className="h-6 w-6 text-primary"/><h3 className="mt-4 font-semibold">För tränare & instruktörer</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">Förbered pass, återanvänd uppställningar och bygg tydliga sekvenser för gruppträning.</p></div><div className="rounded-3xl border border-border bg-card p-6"><Smartphone className="h-6 w-6 text-primary"/><h3 className="mt-4 font-semibold">För förare</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">Spara inspiration, förstå linjer och ta med en tydlig karta till träningsplanen.</p></div></div></div></div></section>
