    ...
    <V3Page>
      ...
      <section className="rounded-[28px] bg-v3-canvas-elevated border border-v3-canvas-sunken/40 p-6 lg:p-8 shadow-v3-xs">
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1"><div className="text-[10px] uppercase tracking-[0.08em] font-medium text-v3-text-tertiary">Klubb</div><h1 className="font-v3-display text-[34px] lg:text-[48px] leading-tight text-v3-text-primary mt-1">{club.name}</h1>{club.city && <p className="inline-flex items-center gap-1 text-v3-sm text-v3-text-secondary mt-2"><MapPin size={14} /> {club.city}</p>}</div>
...
function V3Stat({ label, value, hint }: { label: string; value: string | number; hint?: string }) { return <div className="p-4 rounded-v3-xl border border-v3-canvas-sunken/40 bg-v3-canvas-elevated"><p className="text-[10px] uppercase tracking-[0.08em] text-v3-text-tertiary font-medium">{label}</p><p className="text-2xl font-v3-display text-v3-text-primary mt-1 truncate">{value}</p>{hint && <p className="text-v3-xs text-v3-text-tertiary mt-0.5">{hint}</p>}</div>; }
