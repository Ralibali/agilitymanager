  ...
  return (
    <V3Page>
      <V3PageHero
        eyebrow="Tävlingar"
        title="Mina starter"
        description={active ? `Planera, logga och följ resultaten för ${active.name} utan att tappa bort nästa viktiga steg.` : "Planera starter och samla tävlingshistoriken på ett ställe."}
        icon={Trophy}
      >
        {active && (
          <>
            {tab === "results" && (
              <>
                <V3SecondaryButton onClick={handleExportCsv} icon={Download}>CSV</V3SecondaryButton>
...
function ResultsList({ loading, items, onAdd, onReload, isHoopers, onImport }: { loading: boolean; items: CompetitionResult[]; onAdd: () => void; onReload: () => Promise<void>; isHoopers: boolean; onImport: () => void }) {
  ...
  return <ol className="space-y-2">{items.map((r) => { const isPassed = r.passed && !r.disqualified; return <li key={r.id} className="rounded-v3-xl bg-v3-canvas-elevated border border-v3-canvas-sunken/40 p-4 hover:border-v3-brand-500/25 hover:shadow-v3-xs transition-all group"><div className="flex items-start justify-between gap-3"><div className="min-w-0 flex-1"><div className="flex items-center gap-2 flex-wrap"><span className="text-v3-base font-medium text-v3-text-primary truncate">{(() => { const n = (r.event_name || "").trim(); return !n || n.toLowerCase() === "tävling" ? `${r.sport}-tävling${r.date ? ` · ${formatDate(r.date)}` : ""}` : n; })()}</span>{r.disqualified ? <span className="inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded bg-coral/25 text-coral"><AlertCircle size={10} /> Diskad</span> : isPassed ? <span className="inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded bg-moss/45 text-moss-deep"><CheckCircle2 size={10} /> Godkänd</span> : <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-v3-canvas-sunken text-v3-text-secondary">Ej godkänd</span>}</div>{r.organizer && <div className="text-v3-sm text-v3-text-secondary truncate mt-0.5">{r.organizer}</div>}<div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-2 text-v3-xs text-v3-text-tertiary tabular-nums"><span>{formatDate(r.date)}</span><span aria-hidden>·</span><span>{isHoopers ? r.sport : `${r.discipline} · ${r.competition_level}`}</span>{r.placement && <><span aria-hidden>·</span><span>#{r.placement}</span></>}{r.faults > 0 && <><span aria-hidden>·</span><span>{r.faults} fel</span></>}{r.time_sec > 0 && <><span aria-hidden>·</span><span>{r.time_sec.toFixed(2)} s</span></>}{r.hoopers_points ? <><span aria-hidden>·</span><span>{r.hoopers_points} p</span></> : null}</div></div><button type="button" onClick={() => handleDelete(r.id)} className="h-9 w-9 rounded-full grid place-items-center text-v3-text-tertiary hover:text-coral hover:bg-coral/10 transition-colors opacity-0 group-hover:opacity-100 shrink-0" aria-label="Ta bort"><Trash2 size={13} strokeWidth={1.8} /></button></div></li>; })}</ol>;
}
