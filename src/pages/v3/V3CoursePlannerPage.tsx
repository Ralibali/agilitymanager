...
type PlannerObstacleSpec = {
  type: PlannerObstacleType;
  label: string;
  icon: string;
  shape: "bar" | "wide" | "long" | "circle" | "tunnel" | "zone" | "triangle";
};

type PlannerObstacle = PlannerObstacleSpec & {
  id: string;
  x: number;
  y: number;
  rotation: number;
...
function makeStartObstacles(mode: PlannerMode): PlannerObstacle[] {
  if (mode === "Hoopers") {
    ...
    const hoop = HOOPERS_OBSTACLES.find((o) => o.type === "hoop")!;
    const barrel = HOOPERS_OBSTACLES.find((o) => o.type === "barrel")!;
    const tunnel = HOOPERS_OBSTACLES.find((o) => o.type === "hoopers_tunnel")!;
    const finish = HOOPERS_OBSTACLES.find((o) => o.type === "finish")!;
    return [
      { ...start, id: "planner-hoopers-start", x: 14, y: 82, rotation: 0 },
      { ...hoop, id: "planner-hoopers-1", x: 31, y: 69, rotation: 0, number: 1 },
      { ...barrel, id: "planner-hoopers-2", x: 61, y: 55, rotation: 0, number: 2 },
      { ...tunnel, id: "planner-hoopers-3", x: 46, y: 34, rotation: -12, number: 3 },
      { ...finish, id: "planner-hoopers-finish", x: 82, y: 18, rotation: 0 },
    ];
  }

  const start = AGILITY_OBSTACLES.find((o) => o.type === "start")!;
  const jump = AGILITY_OBSTACLES.find((o) => o.type === "jump")!;
  const tunnel = AGILITY_OBSTACLES.find((o) => o.type === "tunnel")!;
  const weave = AGILITY_OBSTACLES.find((o) => o.type === "weave")!;
  const finish = AGILITY_OBSTACLES.find((o) => o.type === "finish")!;
  return [
    { ...start, id: "planner-start", x: 14, y: 82, rotation: 0 },
    { ...jump, id: "planner-1", x: 32, y: 68, rotation: 0, number: 1 },
    { ...tunnel, id: "planner-2", x: 68, y: 47, rotation: 14, number: 2 },
    { ...weave, id: "planner-3", x: 48, y: 33, rotation: -16, number: 3 },
    { ...finish, id: "planner-finish", x: 82, y: 18, rotation: 0 },
  ];
}
...
export default function V3CoursePlannerPage() {
  ...
  const [obstacles, setObstacles] = useState<PlannerObstacle[]>(() => makeStartObstacles("Agility"));
  ...
  const switchMode = (nextMode: PlannerMode) => {
    ...
    const start = makeStartObstacles(nextMode);
    setMode(nextMode);
  ...
  const addObstacle = (type: PlannerObstacleType = selectedTool) => {
    ...
    const next: PlannerObstacle = {
      ...spec,
      id: `${spec.type}-${Date.now()}`,
      x: 14 + ((obstacles.length * 11) % 72),
      y: 18 + ((obstacles.length * 9) % 66),
      rotation: spec.type.includes("tunnel") ? 18 : 0,
  ...
  const moveSelected = (dx: number, dy: number) => {
    ...
    setObstacles((items) => items.map((item) => item.id === selectedId ? { ...item, x: clamp(item.x + dx, 5, 95), y: clamp(item.y + dy, 5, 95) } : item));
  };
  ...
  const reset = () => {
    const start = makeStartObstacles(mode);
    setObstacles(start);
    setSelectedId(start.find((item) => item.number)?.id ?? start[0]?.id ?? null);
  ...
  return (
    <div className="min-h-[100dvh] bg-[#f7f6f0] text-v3-text-primary pb-[92px] lg:pb-8 animate-v3-fade-in">
      ...
      <main className="max-w-[1400px] mx-auto px-3 lg:px-8 py-3 lg:py-6 space-y-3 lg:space-y-0 lg:grid lg:grid-cols-[280px_minmax(0,1fr)_300px] lg:gap-5 lg:items-start">
        <section className="rounded-[22px] bg-white border border-black/6 shadow-v3-sm p-3 lg:p-5 lg:order-1">
          <div className="flex items-start gap-2 mb-3 lg:mb-5">
            ...
            <div className="min-w-0">
              ...
              <p className="text-v3-sm text-v3-text-tertiary mt-0.5">Läge: <strong>{mode}</strong>. Alla hinder från gamla banplaneraren finns kvar.</p>
            </div>
          </div>
          <div className="hidden lg:block rounded-v3-xl bg-v3-brand-500/10 border border-v3-brand-500/15 p-4 text-v3-sm text-v3-text-secondary">
            <strong className="block text-v3-text-primary mb-1">Komplett hinderutbud</strong>
            {mode === "Hoopers" ? "Hoop, tunnel, tunna, staket, zon, start och mål." : "Hopp, långhopp, oxer, muren, tunnel, A-hinder, brygga, vipp, balans, slalom, däck, start och mål."}
          </div>
        </section>

        <section className="rounded-[22px] bg-white border border-black/6 shadow-v3-sm p-2 lg:p-4 lg:order-2">
          <GrassField obstacles={obstacles} selectedId={selectedId} draggingId={draggingId} onSelect={(id, type) => { setSelectedId(id); setSelectedTool(type); }} onStartDrag={(id) => { setSelectedId(id); setDraggingId(id); }} onPointerMove={handleFieldPointerMove} onPointerUp={() => setDraggingId(null)} />
        </section>
        ...
        <aside className="space-y-3 lg:order-3">
          <section className={cn("rounded-[22px] bg-white border border-black/6 shadow-v3-sm p-3 lg:p-4", mobilePanel === "tools" && "hidden lg:block")}>
            ...
            <div className="grid grid-cols-3 lg:grid-cols-2 gap-2 max-h-[55vh] overflow-y-auto pr-1">
              {obstacleTypes.map((item) => {
                const isActive = selectedTool === item.type;
                return (
                  <button key={item.type} type="button" onClick={() => addObstacle(item.type)} className={cn("min-h-[68px] rounded-v3-xl border p-2 text-center transition-all", isActive ? "bg-v3-brand-500/10 border-v3-brand-500/25 text-v3-brand-800" : "bg-v3-canvas border-v3-canvas-sunken/50 text-v3-text-secondary hover:border-v3-brand-500/30")}> 
                    <span className="block text-[21px] leading-none">{item.icon}</span>
                    <span className="block mt-1.5 text-[11px] font-semibold leading-tight">{item.label}</span>
                  </button>
                );
              })}
            </div>
          ...
          <section className="hidden lg:block rounded-[22px] bg-v3-text-primary text-white shadow-v3-sm p-4">
            ...
            <p className="text-v3-sm text-white/65 mt-1">Spara/export kopplas i nästa steg till befintliga banor.</p>
            <div className="grid grid-cols-2 gap-2 mt-4"><button type="button" className="mobile-tool-btn bg-white text-v3-text-primary"><Save size={15} /> Spara</button><button type="button" className="mobile-tool-btn bg-white text-v3-text-primary"><Download size={15} /> Export</button></div>
          </section>
        </aside>
      </main>
...
function GrassField({ obstacles, selectedId, draggingId, onSelect, onStartDrag, onPointerMove, onPointerUp }: { obstacles: PlannerObstacle[]; selectedId: string | null; draggingId: string | null; onSelect: (id: string, type: PlannerObstacleType) => void; onStartDrag: (id: string) => void; onPointerMove: (event: React.PointerEvent<HTMLDivElement>) => void; onPointerUp: () => void }) {
  return (
    <div className="relative rounded-[20px] overflow-hidden border border-[#d7e3ca] bg-[#77a856] shadow-inner">
      <div onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerUp} className={cn("relative h-[58vh] min-h-[440px] max-h-[620px] lg:h-[calc(100vh-190px)] lg:min-h-[640px] lg:max-h-[840px] select-none overflow-hidden", draggingId ? "cursor-grabbing" : "cursor-default")} style={{ backgroundColor: "#78a957", backgroundImage: "radial-gradient(circle at 18px 18px, rgba(255,255,255,.08) 1px, transparent 1.8px), linear-gradient(rgba(255,255,255,.32) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,.32) 1px, transparent 1px), linear-gradient(135deg, rgba(255,255,255,.09), rgba(25,77,36,.10))", backgroundSize: "20px 20px, 68px 68px, 68px 68px, 100% 100%", touchAction: "none" }}>
...
function getObstacleSize(shape: PlannerObstacleSpec["shape"]) {
  if (shape === "long") return "h-8 w-24 lg:h-9 lg:w-28 rounded-full";
  if (shape === "wide") return "h-8 w-16 lg:h-9 lg:w-20 rounded-lg";
  if (shape === "tunnel") return "h-9 w-16 lg:h-10 lg:w-20 rounded-full";
  if (shape === "circle") return "h-10 w-10 lg:h-11 lg:w-11 rounded-full";
  if (shape === "zone") return "h-16 w-16 lg:h-20 lg:w-20 rounded-lg bg-white/40 border-dashed";
  if (shape === "triangle") return "h-10 w-10 lg:h-11 lg:w-11 rounded-lg";
  return "h-9 w-9 lg:h-10 lg:w-10 rounded-lg";
}

function PlannerGuide({ onClose }: { onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-[80] bg-v3-text-primary/45 backdrop-blur-sm px-4 py-6 flex items-center justify-center"><section className="w-full max-w-md rounded-[26px] bg-white shadow-v3-xl border border-black/10 overflow-hidden animate-v3-fade-up"><div className="p-5 border-b border-black/5 flex items-start justify-between gap-3"><div><div className="text-[10px] uppercase tracking-[0.1em] font-semibold text-v3-text-tertiary">Snabbguide</div><h2 className="font-v3-display text-[28px] leading-tight text-v3-text-primary mt-1">Så bygger du en bana</h2></div><button type="button" onClick={onClose} className="h-10 w-10 rounded-full bg-v3-canvas grid place-items-center text-v3-text-secondary hover:bg-v3-canvas-sunken"><X size={18} /></button></div><div className="p-5 space-y-3"><GuideStep number="1" title="Välj Agility eller Hoopers" text="Läget byter till rätt kompletta hinderutbud." /><GuideStep number="2" title="Lägg till hinder" text="Tryck på Hinder eller välj ett hinderkort i panelen." /><GuideStep number="3" title="Dra på gräset" text="Håll inne på ett hinder och dra det till rätt plats på planen." /><GuideStep number="4" title="Öppna Verktyg" text="Använd Verktyg för att rotera, finjustera, ta bort eller återställa banan." /></div><div className="p-5 pt-0"><button type="button" onClick={onClose} className="w-full h-12 rounded-v3-base bg-v3-brand-600 text-white text-v3-sm font-semibold shadow-v3-sm hover:bg-v3-brand-700 transition-colors">Jag fattar – börja rita</button></div></section></div>
  );
}
