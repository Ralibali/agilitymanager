...
import { BarChart3, ChevronDown, Download, FileText, MapPinned, RotateCcw, Ruler, Save, X } from "lucide-react";
...
type AnyObstacle = { id: string; type: string; label?: string; x: number; y: number; rotation?: number; number?: number; curveDeg?: number; curveSide?: "left" | "right" };
...
function safeFilename(value: string) { return value.toLowerCase().replace(/[^a-z0-9åäö]+/gi, "-").replace(/^-+|-+$/g, "") || "bana"; }
...
async function exportBuildPdf(mode: string, course: StoredCourse, speed: number) {
  const { default: jsPDF } = await import("jspdf");
  const pdf = new jsPDF({ orientation: "landscape", unit: "mm", format: "a4" });
  const pageW = pdf.internal.pageSize.getWidth();
  const pageH = pdf.internal.pageSize.getHeight();
  const margin = 14;
  const title = course.name || `${mode}-bana`;
  const width = course.width ?? 40;
  const height = course.height ?? 20;
  const rows = orderedObstacles(course);
  const numbered = numberedObstacles(course);
  const length = estimateCourseLength(course);
  const estTime = speed > 0 ? Math.ceil(length / speed) : 0;
  const maxFieldW = 170;
  const maxFieldH = 118;
  const ratio = width / height;
  const fieldW = ratio >= maxFieldW / maxFieldH ? maxFieldW : maxFieldH * ratio;
  const fieldH = ratio >= maxFieldW / maxFieldH ? maxFieldW / ratio : maxFieldH;
  const fieldX = margin;
  const fieldY = 42;
  const toPdf = (o: AnyObstacle) => ({ x: fieldX + (o.x / 100) * fieldW, y: fieldY + (o.y / 100) * fieldH });

  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(18);
  pdf.text("Bygg-PDF", margin, 18);
  pdf.setFontSize(11);
  pdf.setFont("helvetica", "normal");
  pdf.text(`${title} · ${width} × ${height} m · ${Math.round(length)} m · ca ${estTime} s vid ${speed.toFixed(1)} m/s`, margin, 26);
  pdf.setDrawColor(25, 55, 38);
  pdf.setFillColor(238, 244, 232);
  pdf.roundedRect(fieldX, fieldY, fieldW, fieldH, 2, 2, "FD");
  pdf.setDrawColor(210, 220, 205);
  pdf.setLineWidth(0.15);
  for (let x = 0; x <= width; x += 5) { const px = fieldX + (x / width) * fieldW; pdf.line(px, fieldY, px, fieldY + fieldH); }
  for (let y = 0; y <= height; y += 5) { const py = fieldY + (y / height) * fieldH; pdf.line(fieldX, py, fieldX + fieldW, py); }
  pdf.setDrawColor(45, 75, 54);
  pdf.setLineWidth(0.5);
  pdf.rect(fieldX, fieldY, fieldW, fieldH);

  pdf.setDrawColor(32, 36, 95);
  pdf.setLineWidth(0.7);
  for (let i = 1; i < numbered.length; i++) {
    const a = toPdf(numbered[i - 1]);
    const b = toPdf(numbered[i]);
    pdf.line(a.x, a.y, b.x, b.y);
  }

  for (const obstacle of rows) {
    const p = toPdf(obstacle);
    const isT = isTunnel(obstacle.type);
    pdf.setFillColor(isT ? 37 : 255, isT ? 99 : 255, isT ? 235 : 255);
    pdf.setDrawColor(23, 61, 44);
    if (isT && (obstacle.curveDeg ?? 0) > 0) {
      const side = obstacle.curveSide === "right" ? 1 : -1;
      const bend = ((obstacle.curveDeg ?? 0) / 90) * 8 * side;
      pdf.setLineWidth(3.2);
      pdf.setDrawColor(37, 99, 235);
      // jsPDF quadratic curve support via path API is not stable across versions; approximate with two segments.
      pdf.line(p.x - 4, p.y, p.x, p.y + bend);
      pdf.line(p.x, p.y + bend, p.x + 4, p.y);
      pdf.setLineWidth(0.7);
    } else {
      pdf.circle(p.x, p.y, isT ? 3.2 : 2.5, "FD");
    }
    pdf.setFont("helvetica", "bold");
    pdf.setFontSize(7);
    pdf.setTextColor(0, 0, 0);
    pdf.text(String(obstacle.number ?? ""), p.x + 3.5, p.y - 2.8);
  }

  const tableX = fieldX + fieldW + 12;
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(12);
  pdf.text("Bygglista", tableX, fieldY);
  pdf.setFont("helvetica", "normal");
  pdf.setFontSize(8);
  let y = fieldY + 7;
  pdf.text("#", tableX, y); pdf.text("Hinder", tableX + 10, y); pdf.text("X/Y m", tableX + 58, y); pdf.text("Rot", tableX + 88, y); pdf.text("Tunnel", tableX + 104, y);
  y += 3;
  pdf.setDrawColor(220, 220, 220); pdf.line(tableX, y, pageW - margin, y); y += 5;
  for (const obstacle of rows.slice(0, 28)) {
    const p = meterPoint(course, obstacle);
    pdf.text(String(obstacle.number ?? "–"), tableX, y);
    pdf.text(String(obstacle.label ?? obstacle.type).slice(0, 18), tableX + 10, y);
    pdf.text(`${p.x.toFixed(1)} / ${p.y.toFixed(1)}`, tableX + 58, y);
    pdf.text(`${Math.round(obstacle.rotation ?? 0)}°`, tableX + 88, y);
    pdf.text(isTunnel(obstacle.type) ? `${obstacle.curveDeg ?? 0}° ${obstacle.curveSide ?? ""}` : "", tableX + 104, y);
    y += 5.2;
    if (y > pageH - 16) break;
  }
  pdf.setTextColor(100, 100, 100);
  pdf.setFontSize(7);
  pdf.text("Banlängd är uppskattad som rak linje mellan numrerade hinder. Kontrollera alltid mot faktisk hundlinje.", margin, pageH - 9);
  pdf.save(`${safeFilename(title)}-bygg-pdf.pdf`);
}
...
function JudgeToolsPanel() {
  ...
  return <div className="fixed left-3 bottom-[calc(env(safe-area-inset-bottom)+1rem)] z-[1049] w-[min(430px,calc(100vw-1.5rem))] font-v3-sans">{!open ? <button type="button" onClick={() => setOpen(true)} className="flex h-12 items-center gap-2 rounded-full bg-[#20245f] px-4 text-sm font-semibold text-white shadow-2xl"><Ruler size={16} /> Domarvy / mät</button> : <div className="max-h-[78vh] overflow-hidden rounded-[28px] border border-black/10 bg-white/95 shadow-2xl backdrop-blur-xl"><div className="flex items-start justify-between gap-3 border-b border-black/10 p-4"><div><div className="text-[11px] font-bold uppercase tracking-[0.12em] text-v3-text-tertiary">Course design</div><div className="text-lg font-semibold text-v3-text-primary">Domarvy & bygglista</div><p className="text-xs text-v3-text-secondary">Mått, koordinater, banlängd och tid.</p></div><button type="button" onClick={() => setOpen(false)} className="grid h-8 w-8 place-items-center rounded-full bg-black/5"><X size={15} /></button></div><div className="max-h-[calc(78vh-86px)] overflow-auto p-4"><label className="mb-1 block text-xs font-semibold text-v3-text-secondary">Bana</label><select value={mode} onChange={(e) => setMode(e.target.value)} className="mb-3 h-10 w-full rounded-2xl border border-black/10 bg-white px-3 text-sm font-medium outline-none focus:ring-2 focus:ring-[#20245f]/20">{courseOptions.map((item) => <option key={item.mode} value={item.mode}>{item.mode}: {item.course.name ?? `${item.mode}-bana`}</option>)}</select><div className="grid grid-cols-3 gap-2 text-center"><Metric icon={MapPinned} label="Storlek" value={`${course.width ?? 40}×${course.height ?? 20} m`} /><Metric icon={Ruler} label="Banlängd" value={`${Math.round(courseLength)} m`} /><Metric icon={BarChart3} label="Tid" value={`${Math.ceil(courseTime)} s`} /></div><button type="button" onClick={() => exportBuildPdf(mode, course, speed).then(() => toast.success("Bygg-PDF exporterad")).catch(() => toast.error("Kunde inte exportera bygg-PDF"))} className="mt-3 inline-flex h-11 w-full items-center justify-center gap-2 rounded-2xl bg-[#20245f] text-sm font-semibold text-white"><Download size={15} /> Exportera Bygg-PDF</button><div className="mt-4 rounded-3xl border border-black/10 bg-[#f7f6f0] p-3"><div className="mb-2 flex items-center gap-2 text-sm font-semibold text-v3-text-primary"><Ruler size={15} /> Mät mellan hinder</div><div className="grid grid-cols-2 gap-2"><select value={measureA?.id ?? ""} onChange={(e) => setAId(e.target.value)} className="h-9 rounded-2xl border border-black/10 bg-white px-2 text-xs">{rows.map((o) => <option key={o.id} value={o.id}>{o.number ? `#${o.number} ` : ""}{o.label ?? o.type}</option>)}</select><select value={measureB?.id ?? ""} onChange={(e) => setBId(e.target.value)} className="h-9 rounded-2xl border border-black/10 bg-white px-2 text-xs">{rows.map((o) => <option key={o.id} value={o.id}>{o.number ? `#${o.number} ` : ""}{o.label ?? o.type}</option>)}</select></div><div className="mt-2 rounded-2xl bg-white px-3 py-2 text-sm font-semibold text-v3-text-primary">Avstånd: {measureDistance.toFixed(1)} m</div></div><div className="mt-4 rounded-3xl border border-black/10 bg-white p-3"><div className="mb-2 flex items-center justify-between gap-2"><div className="flex items-center gap-2 text-sm font-semibold"><FileText size={15} /> Bygglista</div><label className="flex items-center gap-2 text-xs text-v3-text-secondary">m/s<input type="number" min="1" max="8" step="0.1" value={speed} onChange={(e) => setSpeed(Number(e.target.value) || 0)} className="h-8 w-16 rounded-xl border border-black/10 px-2" /></label></div><div className="overflow-hidden rounded-2xl border border-black/10"><table className="w-full text-left text-[11px]"><thead className="bg-black/5 text-v3-text-tertiary"><tr><th className="p-2">#</th><th className="p-2">Hinder</th><th className="p-2 text-right">X/Y m</th><th className="p-2 text-right">Rot.</th></tr></thead><tbody>{rows.map((o) => { const p = meterPoint(course, o); return <tr key={o.id} className="border-t border-black/5"><td className="p-2 font-bold">{o.number ?? "–"}</td><td className="p-2">{o.label ?? o.type}</td><td className="p-2 text-right tabular-nums">{p.x.toFixed(1)} / {p.y.toFixed(1)}</td><td className="p-2 text-right tabular-nums">{Math.round(o.rotation ?? 0)}°</td></tr>; })}</tbody></table></div><p className="mt-2 text-[11px] leading-snug text-v3-text-tertiary">Banlängden är en uppskattning mellan numrerade hinder. Bygg-PDF innehåller fältskiss, koordinater, rotation och tunnelböjning.</p></div></div></div>}</div>;
}
...
export default function V3CoursePlannerPageTunnelIntegrated() { return <><V3CoursePlannerPageFixed /><JudgeToolsPanel /><TunnelCurvePanel /></>; }
