...
function CatalogTab({ courses, allCount, purchases, isPremium, recs, categories, activeCategory, onCategory, onSelect, priceFor }: { courses: V3Course[]; allCount: number; purchases: Set<string>; isPremium: boolean; recs: { items: V3Course[]; reason: string }; categories: string[]; activeCategory: string; onCategory: (c: string) => void; onSelect: (c: V3Course) => void; priceFor: (c: V3Course) => number }) {
  return (
    <div className="space-y-8">
      {!isPremium && <div className="rounded-v3-xl bg-v3-accent-traning/10 border border-v3-accent-traning/30 p-4 flex items-center gap-3"><div className="w-9 h-9 rounded-v3-base bg-v3-accent-traning/20 flex items-center justify-center"><Star size={16} className="text-v3-accent-traning" /></div><div className="flex-1 min-w-0"><div className="text-v3-sm font-medium text-v3-text-primary">Pro-medlemmar får rabatt på alla kurser</div><div className="text-v3-sm text-v3-text-secondary">{PLANS.monthly.price}. Aktivera och börja spara direkt.</div></div><Link to="/v3/settings#pro-prenumeration" className="v3-tappable rounded-v3-base bg-v3-text-primary text-v3-canvas px-3.5 py-2 text-v3-sm font-medium">Bli Pro</Link></div>}
