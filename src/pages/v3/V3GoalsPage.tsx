...
export default function V3GoalsPage() {
  ...
  const toggleComplete = async (goal: V3Goal) => {
    ...
    const { error } = await supabase.from("training_goals").update({ status: newStatus }).eq("id", goal.id);
    if (error) {
  ...
  return (
    <V3Page>
      <V3PageHero
        eyebrow="Mål & badges"
        title="Sikta mot nästa."
        description={active ? `Sätt tydliga mål för ${active.name}, följ utvecklingen och lås upp badges när ni gör framsteg.` : "Sätt mål, följ utvecklingen och lås upp badges när ni gör framsteg."}
        icon={Target}
      >
        {active && <V3PrimaryButton onClick={openNew} icon={Plus}>Nytt mål</V3PrimaryButton>}
      </V3PageHero>
        ...
        <>
          ...
          <div className="flex gap-1.5 border-b border-v3-canvas-sunken/40">
            {FILTERS.map(({ value, label }) => {
              ...
              return (
                <button key={value} type="button" onClick={() => setFilter(value)} className={cn("h-11 px-4 -mb-px text-v3-sm font-medium transition-colors border-b-2", isActive ? "text-v3-text-primary border-v3-text-primary" : "text-v3-text-tertiary border-transparent hover:text-v3-text-secondary")}>{label}</button>
              );
            })}
          </div>

          <GoalsList loading={loading} items={filtered} onAdd={openNew} onToggle={toggleComplete} onEdit={openEdit} onDelete={removeGoal} filter={filter} dogName={active.name} />
      ...
      <V3AddGoalSheet open={sheet} onClose={() => { setSheet(false); setEditing(null); }} dog={active} editing={editing} onSaved={reload} />
    </V3Page>
  );
}
...
function GoalsList({ loading, items, onAdd, onToggle, onEdit, onDelete, filter, dogName }: { loading: boolean; items: V3Goal[]; onAdd: () => void; onToggle: (g: V3Goal) => void; onEdit: (g: V3Goal) => void; onDelete: (id: string) => void; filter: Filter; dogName: string }) {
  if (loading) {
    return <div className="space-y-2">{[0, 1, 2].map((i) => <div key={i} className="h-24 rounded-v3-lg v3-skeleton" />)}</div>;
  }

  if (items.length === 0) {
    return (
      <div className="rounded-v3-2xl bg-v3-canvas-elevated border border-v3-canvas-sunken/40 p-8 text-center">
        <div className="mx-auto h-12 w-12 rounded-full bg-v3-brand-500/10 grid place-items-center mb-4"><Target size={20} strokeWidth={1.6} className="text-v3-brand-500" /></div>
        <h3 className="font-v3-display text-v3-2xl text-v3-text-primary">{filter === "completed" ? `Inga avklarade mål för ${dogName} än` : `Inga mål för ${dogName} än`}</h3>
        <p className="text-v3-base text-v3-text-secondary mt-2 max-w-sm mx-auto">{filter === "completed" ? "När du markerar mål som klara samlas de här." : `Sätt riktning för ${dogName}s träning — till exempel 10 nollrundor eller klass 2 till sommaren.`}</p>
        {filter !== "completed" && <button type="button" onClick={onAdd} className="mt-5 inline-flex items-center gap-2 h-11 px-5 rounded-v3-base bg-v3-brand-500 text-white text-v3-sm font-medium hover:bg-v3-brand-600 transition-colors shadow-v3-sm"><Plus size={16} strokeWidth={2} />Skapa mål</button>}
      </div>
    );
  }
        ...
        return (
          <li key={g.id} className={cn("rounded-v3-lg bg-v3-canvas-elevated border border-v3-canvas-sunken/40 p-4 hover:border-v3-canvas-sunken transition-colors group", isDone && "opacity-70")}>
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap"><span className={cn("text-v3-base text-v3-text-primary truncate", isDone && "line-through")}>{g.title}</span><CategoryChip category={g.category} />{isDone && <span className="inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded bg-moss/45 text-moss-deep dark:bg-emerald-950/40 dark:text-emerald-400"><CheckCircle2 size={10} /> Klart</span>}</div>
...
function CategoryChip({ category }: { category: string }) {
  return <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-v3-canvas-sunken text-v3-text-secondary">{category}</span>;
}
