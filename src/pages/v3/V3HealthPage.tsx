            ...
            <section className="space-y-3">
              ...
              <div className="grid lg:grid-cols-2 gap-3">
                {stats.overdueReminders.map((r) => (
                  <ReminderCard key={r.id} log={r} overdue />
                ))}
                {stats.upcomingReminders.map((r) => (
                  <ReminderCard key={r.id} log={r} />
                ))}
              </div>
            </section>
          )}
    ...
    >
      ...
      <div className="font-v3-display text-[28px] leading-none mt-2 text-v3-text-primary tabular-nums truncate flex items-baseline gap-1.5">
        ...
        {trend !== null && trend !== undefined && (
          <span
            className={cn(
              "text-v3-xs font-sans inline-flex items-center gap-0.5",
              trend > 0 ? "text-rose-600" : trend < 0 ? "text-emerald-600" : "text-v3-text-tertiary",
            )}
          >
...
function ReminderCard({ log, overdue }: { log: V3HealthLog; overdue?: boolean }) {
  ...
  return (
    <div
      className={cn(
        "rounded-v3-lg border p-4 flex items-start gap-3",
        overdue
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="text-v3-sm font-medium text-v3-text-primary truncate">
          {meta.emoji} {log.title}
        </div>
        <div className="text-v3-xs text-v3-text-tertiary mt-0.5">
          {overdue ? "Försenad" : days === 0 ? "Idag" : days === 1 ? "Imorgon" : `Om ${days} dagar`} ·{" "}
          {formatDate(log.next_date)}
        </div>
      </div>
    </div>
  );
...
function WeightChart({ series }: { series: { date: string; weight: number }[] }) {
  ...
  const points = series.map((s, i) => {
    ...
    const y = padY + (1 - (s.weight - min) / range) * (H - padY * 2);
    return { x, y, ...s };
  });
  ...
  return (
    <div className="rounded-v3-xl bg-v3-canvas-elevated border border-v3-canvas-sunken/40 p-4">
      <div className="flex items-end justify-between mb-3">
        <div>
          <div className="text-[10px] uppercase tracking-[0.08em] font-medium text-v3-text-tertiary">
            {series.length} mätningar
          </div>
          <div className="text-v3-sm text-v3-text-secondary mt-0.5 tabular-nums">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" preserveAspectRatio="none">
        <defs>
          <linearGradient id="weight-grad" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="hsl(var(--v3-accent-halsa))" stopOpacity="0.25" />
            <stop offset="100%" stopColor="hsl(var(--v3-accent-halsa))" stopOpacity="0" />
          </linearGradient>
        </defs>
        ...
        {points.map((p, i) => (
          <circle key={i} cx={p.x} cy={p.y} r={3} fill="hsl(var(--v3-accent-halsa))" />
        ))}
      </svg>
      <div className="flex justify-between text-v3-xs text-v3-text-tertiary mt-2 tabular-nums">
    ...
    return (
      <div className="rounded-v3-xl border border-dashed border-v3-canvas-sunken/60 bg-v3-canvas-elevated/40 p-10 text-center space-y-3">
        ...
        <p className="text-v3-sm text-v3-text-tertiary max-w-sm mx-auto">
          Logga vetbesök, vaccinationer och vikt för att hålla koll på din hunds hälsa över tid.
        </p>
        <button
          type="button"
          onClick={onAdd}
...
function LogRow({
  ...
  return (
    <div className="rounded-v3-lg bg-v3-canvas-elevated border border-v3-canvas-sunken/40 p-4 flex items-start gap-3 hover:bg-v3-canvas-elevated/80 transition-colors">
      <div className="shrink-0 h-11 w-11 rounded-full grid place-items-center bg-v3-canvas text-2xl">
        {meta.emoji}
      </div>
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 flex-wrap">
          <div className="text-v3-sm font-medium text-v3-text-primary truncate">{log.title}</div>
          <span className="text-[10px] uppercase tracking-wide font-medium text-v3-text-tertiary px-1.5 py-0.5 rounded bg-v3-canvas">
            {meta.label}
          </span>
        </div>
        {log.description && (
          <div className="text-v3-xs text-v3-text-secondary mt-1 line-clamp-2">{log.description}</div>
        )}
        <div className="text-v3-xs text-v3-text-tertiary mt-1.5 flex items-center gap-3 flex-wrap tabular-nums">
          <span>{formatDate(log.date)}</span>
          {log.weight_kg && <span>· {Number(log.weight_kg).toFixed(1)} kg</span>}
          {log.next_date && <span>· nästa {formatDate(log.next_date)}</span>}
        </div>
      </div>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button
            type="button"
            className="p-2 rounded-full text-v3-text-tertiary hover:text-v3-text-primary hover:bg-v3-canvas transition shrink-0"
        ...
        <DropdownMenuContent align="end" className="w-44">
          <DropdownMenuItem onClick={() => onEdit(log)}>
            <Pencil size={14} className="mr-2" /> Redigera
          </DropdownMenuItem>
          <DropdownMenuItem
            className="text-rose-600 focus:text-rose-600"
            onClick={() => {
              if (confirm("Ta bort denna logg?")) onDelete(log.id);
            }}
          >
            <Trash2 size={14} className="mr-2" /> Ta bort
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
