...
function Metric({ icon: Icon, label, value, sub, tone }: { icon: LucideIcon; label: string; value: string; sub: string; tone: "green" | "warm" | "neutral" }) {
  ...
  return <section className="rounded-v3-2xl bg-v3-canvas-elevated border border-v3-canvas-sunken/40 p-5 shadow-v3-xs min-h-[150px]"><div className="flex items-start justify-between gap-3"><div className="text-[10px] uppercase tracking-[0.08em] font-medium text-v3-text-tertiary">{label}</div><div className={cn("h-9 w-9 rounded-full grid place-items-center", toneClass)}><Icon size={16} strokeWidth={1.8} /></div></div><div className="font-v3-display text-[34px] leading-none mt-5 text-v3-text-primary tabular-nums truncate">{value}</div><div className="text-v3-sm text-v3-text-tertiary mt-1">{sub}</div></section>;
}
