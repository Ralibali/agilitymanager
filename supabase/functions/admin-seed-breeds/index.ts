// One-shot admin upsert function for breed seeding.
// Reads a JSONB array of breed records from the request body and upserts them
// using the service role (bypasses RLS).
//
// Auth: protected by a fixed admin token in the SEED_ADMIN_TOKEN secret —
// MUST be sent as the `x-seed-token` header.
...
Deno.serve(async (req) => {
  ...
  // Auth via shared secret. We don't accept regular JWT auth here because this
  // is only meant to be invoked once during initial seeding from the build env.
  ...
  // Map each row to the public.breeds schema explicitly so unknown keys are
  // ignored and missing optional keys default sensibly.
  ...
  return new Response(
    JSON.stringify({ ok: true, count: data?.length ?? 0, slugs: data?.map((r) => r.slug) }),
    { headers: { ...corsHeaders, "content-type": "application/json" } },
  );
});
